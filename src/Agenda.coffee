class Agenda
  ...
  build: (presentation) ->
    ...
    for chapterIndex in [0..widths.length - 1]
      for slideIndex in [0..widths[chapterIndex].length - 1]
        if presentation.chapters[chapterIndex].slides[slideIndex].title
          title = presentation.chapters[chapterIndex].slides[slideIndex].title
        else
  ...
  select: (presentation, chapterIndex, slideIndex) ->
    ...
    for index in [0..chapterIndex - 1] when chapterIndex - 1 >= 0
      currentSlideIndex += presentation.chapters[index].slides.length
